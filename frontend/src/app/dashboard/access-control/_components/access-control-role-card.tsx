...
export function AccessControlRoleCard({
  icon: Icon,
  iconSrc,
  title,
  description,
  ...
  return (
    ...
    >
      ...
      >
        {Icon ? (
          <Icon className={`size-10 ${classes.icon}`} />
        ) : iconSrc ? (
          <Image
            src={iconSrc}
            alt={title}
      ...
      <div className="space-y-4 pt-6">
        <CardHeader className="gap-1 p-0">
          <CardTitle className="text-lg font-bold text-heading-lg">
            {title}
          </CardTitle>
          <CardDescription className="text-xs text-heading-md">
            {description}
          </CardDescription>
        </CardHeader>

        <div className="flex items-center justify-between rounded-sm bg-accent p-2">
          ...
          <Link
            prefetch={false}
            href={detailUrl}
            className="inline-flex items-center gap-1 text-xs font-medium text-primary-foreground hover:underline"
          >
            Chi tiết <ArrowRight className="size-3 text-primary-foreground" />
          </Link>
        </div>
      </div>
    </Card>
